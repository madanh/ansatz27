...
classdef JSON < handle
    ...
    methods (Static)
        ...
        function uri = resolveURIagainstLoadPath(uri)
            % resolveURIagainstLoadPath resolves the reference against the base URI
            % and normalizes. See https://tools.ietf.org/html/rfc3986
            ...
            if isempty(r.getScheme())
                ...
                % We should not rely on which() for relative paths, it can only resolve
                % filenames without path.  Let's resolve properly against the current
                % path instead.
                
                % don't forget the trailing '/', otherwise it's not a path
                base = [ 'file:///', strrep( pwd, '\', '/'), '/'];
                uri = char( JSON.resolveURI( r, base));
                
            end
        end
        
        
        function resolvedURI = resolveURI( uri, base)
            ...
            if ~isempty(base)
                ...
                % Workaround for a Java bug. Resolved uri may look like
                % file://foo/bar. We need four slashes after the protocol.
        ...
        function [obj, chain] = getPath(obj, pointer, default)
            ...
            % tokens = strsplit(pointer, '/');
            % this is 10x faster than strsplit()
            tokens = regexp( pointer, '(?:/)+', 'split');
            nTok = length( tokens);
            % minor speedup by preallocation
            chain = cell( 1, nTok - 1);
